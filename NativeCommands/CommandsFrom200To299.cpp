...
namespace Natives
{
    ...
    {
        CTheScripts::StartNewScript(reinterpret_cast<unsigned char*>(&CTheScripts::ScriptSpace[MissionLabel_LocalOffset]));
    }
    ...
    {
        if (pPed->m_nPedFlags.bInVehicle && pPed->m_pVehicle == pVehicle) 
            return true;
    ...
    {
        if (pPed->m_nPedFlags.bInVehicle && pPed->m_pVehicle)
        {
            return true;
        }
    ...
    bool IS_BUTTON_PRESSED(unsigned short playerid, eButtonID buttonID)
    {
        ...
        if ((plugin::CallMethodAndReturnDynGlobal<short, CRunningScript *, unsigned short, unsigned short>
            (gaddrof(CRunningScript::GetPadState), 0, playerid, buttonID)) 
    ...
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_ANY_MEANS,false,false>(pPed, (CVector&)posn, (CVector&)radius, bSphere);
    }

    bool LOCATE_CHAR_ON_FOOT_2D(CPed* pPed, CVector2D& posn, CVector2D& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_ON_FOOT, false, false>(pPed, (CVector&)posn, (CVector&)radius, bSphere);
    }

    bool LOCATE_CHAR_IN_CAR_2D(CPed* pPed, CVector2D& posn, CVector2D& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_IN_CAR, false, false>(pPed, (CVector&)posn, (CVector&)radius, bSphere);
    }

    bool LOCATE_STOPPED_CHAR_ANY_MEANS_2D(CPed* pPed, CVector2D& posn, CVector2D& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_ANY_MEANS, false, true>(pPed, (CVector&)posn, (CVector&)radius, bSphere);
    }

    bool LOCATE_STOPPED_CHAR_ON_FOOT_2D(CPed* pPed, CVector2D& posn, CVector2D& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_ON_FOOT, false, true>(pPed, (CVector&)posn, (CVector&)radius, bSphere);
    }

    bool LOCATE_STOPPED_CHAR_IN_CAR_2D(CPed* pPed, CVector2D& posn, CVector2D& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_IN_CAR, false, true>(pPed, (CVector&)posn, (CVector&)radius, bSphere);
    }

    bool LOCATE_CHAR_ANY_MEANS_CHAR_2D(CPed* pPed, CPed* pTargetPed, CVector2D& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCharCommand<eLocateCommand::LOCATE_ANY_MEANS, false>(pPed, pTargetPed, (CVector&)radius, bSphere);
    }

    bool LOCATE_CHAR_ON_FOOT_CHAR_2D(CPed* pPed, CPed* pTargetPed, CVector2D& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCharCommand<eLocateCommand::LOCATE_ON_FOOT, false>(pPed, pTargetPed, (CVector&)radius, bSphere);
    }

    bool LOCATE_CHAR_IN_CAR_CHAR_2D(CPed* pPed, CPed* pTargetPed, CVector2D& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCharCommand<eLocateCommand::LOCATE_IN_CAR, false>(pPed, pTargetPed, (CVector&)radius, bSphere);
    }

    bool LOCATE_CHAR_ANY_MEANS_3D(CPed* pPed, CVector& posn, CVector& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_ANY_MEANS, true, false>(pPed, posn,radius, bSphere);
    }

    bool LOCATE_CHAR_ON_FOOT_3D(CPed* pPed, CVector& posn, CVector& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_ON_FOOT, true, false>(pPed, posn, radius, bSphere);
    }

    bool LOCATE_CHAR_IN_CAR_3D(CPed* pPed, CVector& posn, CVector& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_IN_CAR, true, false>(pPed, posn, radius, bSphere);
    }


    bool LOCATE_STOPPED_CHAR_ANY_MEANS_3D(CPed* pPed, CVector& posn, CVector& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_ANY_MEANS, true, true>(pPed, posn, radius, bSphere);
    }

    bool LOCATE_STOPPED_CHAR_ON_FOOT_3D(CPed* pPed, CVector& posn, CVector& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_ON_FOOT, true, true>(pPed, posn, radius, bSphere);
    }

    bool LOCATE_STOPPED_CHAR_IN_CAR_3D(CPed* pPed, CVector& posn, CVector& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCommand<eLocateCommand::LOCATE_IN_CAR, true, true>(pPed, posn, radius, bSphere);
    }

    bool LOCATE_CHAR_ANY_MEANS_CHAR_3D(CPed* pPed, CPed* pTargetPed, CVector& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCharCommand<eLocateCommand::LOCATE_ANY_MEANS, true>(pPed, pTargetPed, radius, bSphere);
    }

    bool LOCATE_CHAR_ON_FOOT_CHAR_3D(CPed* pPed, CPed* pTargetPed, CVector& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCharCommand<eLocateCommand::LOCATE_ON_FOOT, true>(pPed, pTargetPed, radius, bSphere);
    }

    bool LOCATE_CHAR_IN_CAR_CHAR_3D(CPed* pPed, CPed* pTargetPed, CVector& radius, bool bSphere)
    {
        return CStaticScript::LocateCharCharCommand<eLocateCommand::LOCATE_IN_CAR, true>(pPed, pTargetPed, radius, bSphere);
    }
    ...
    CObject* CREATE_OBJECT(int modelIndex, CVector& posn,eObjectType objectType , bool bNoOffset)
    {
        ...
        if (rwobject)
        {
            ...
            RwMatrixTag* matrixtag = (RwMatrixTag *)((char *)rwobject->parent + 16);
            if (matrixlink)
            {
    ...
    {
        return (CWorld::Players[playerid].m_nMoney > amount) ? true : false;
    }
    ...
    {
        return CWorld::Players[playerid].m_nPlayerState == ePlayerState::PLAYERSTATE_HASDIED;
    }
    ...
    bool IS_CHAR_DEAD(CPed* pPed)
    {
        ...
        if (pedstate == PEDSTATE_DEAD
            || pedstate == PEDSTATE_DIE
            || pedstate == PEDSTATE_DIE_BY_STEALTH)
            return true;
    ...
    bool IS_CAR_DEAD(CVehicle* pVehicle)
    {
        ...
        if (pVehicle->m_nStatus != eEntityStatus::STATUS_WRECKED)
