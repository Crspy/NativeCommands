...
namespace Natives
{
        ...
        bool bForceSpawn, unsigned char Alarm_chances, unsigned char DoorLock_chances, int delayStart, int delayEnd)
    {
        return CTheCarGenerators::CreateCarGenerator(posn.x, posn.y, posn.z, Heading, modelindex,
            color1_ID, color2_ID, bForceSpawn, Alarm_chances, DoorLock_chances, delayStart, delayEnd, 0, true);
    }
    ...
    void DISPLAY_ONSCREEN_TIMER(int id, eOnScreenTimer direction)
    {
        if (direction)
        {
            CUserDisplay::OnscnTimer.AddClock((unsigned int)&CStaticScript::script.m_aLocalVars[id].iParam, 0, direction);
        }
        else
        {
            CUserDisplay::OnscnTimer.AddClock((unsigned int)&CStaticScript::script.m_aLocalVars[id].iParam, 0, direction);
    ...
    {
        CUserDisplay::OnscnTimer.AddCounter((unsigned int)&CStaticScript::script.m_aLocalVars[id].iParam, countertype, 0, 0);
    }
